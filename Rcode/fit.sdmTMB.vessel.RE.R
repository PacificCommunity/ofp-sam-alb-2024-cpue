...
model.name <- paste0("vessel.re.65-22")
set.seed(1134)

#folds <- seq(1, 20, 1) # folds for cross-validation

load(file = "../Data/subsample_1954.Rdata")
DG <- sub; rm(sub)
## subset for exploration
DG %<>% filter(year >= 1965, year <= 2022)
DG %<>% mutate(Response_variable = alb_cpue,
                          month = as.numeric(month))

## remove vessels with missing IDs
DG %<>% filter(!is.na(vessel_id)) %>% mutate
## remove vessels with less than 10 obs.
keeps <- names(table(DG$vessel_id)[(table(DG$vessel_id)) >= 10])
DG %<>% filter(vessel_id %in% keeps); rm(keeps)
...
coast_sf <- sf::st_as_sf(SP.ALB.coast.shp) 
...
## add equi-distant coordinates to mesh coords
...
## add equidistant coordinates to cpue data
...
DG %<>% mutate(vessel = as.factor(vessel_id))
fmla <- as.formula("Response_variable ~ as.factor(year) + as.factor(flag_id) + s(hbf, bs='bs', m=c(3,2)) + s(month, bs = 'cc', k = 12) + (1 | vessel)")
fmla

  ## may need to turn off library(BlockCV), it can interfere with the mesh funcs
  mesh_sdmTMB = make_mesh(DG, c("lon_eqd", "lat_eqd"), mesh = mesh_inla)
  mesh = add_barrier_mesh(mesh_sdmTMB,coast_eqd_sf,
                          range_fraction = 0.01,plot=FALSE)
  plot(mesh)

    #mesh.df <- mesh$A_st %>% as.matrix()

    #for(i in 1:nrow(mesh.df)){
    #  tmp <- mesh.df[i,] %>% unlist()
    #  
    #  tmp.df <- data.frame(knot_i = which(tmp > 0), prop = tmp[which(tmp > 0)],
    #                       year = DG$year[i], data.row = i)
    #  
    #  if(!exists("final.df")){
    #    final.df <- tmp.df
    #  } else {
    ...
    #  rm(tmp.df, tmp)
    #}
    #knot.yr.samples <- final.df; rm(final.df)

  #knot.yr.samples %<>%
  #  mutate(knot.year = paste0(knot_i, "-", year))
  
    #save(knot.yr.samples, file = "../Data/knot.yr.samples.1960.Rdata")
  #  load(file = "../Data/knot.yr.samples.1960.Rdata")

  # removes <- knot.yr.samples %>% mutate(n = 1) %>%
  #    group_by(knot.year) %>% summarise(n = sum(n)) %>% ungroup() %>%
  #   filter(n == 1) %>% select(knot.year) %>% unlist() %>% unname()
  #  row.keeps <- knot.yr.samples %>% filter(!(knot.year %in% removes)) %>%
  #   select(data.row) %>% distinct() %>% unlist() %>% unname()
  ...
  graphics.off()
  #rm(list=setdiff(ls(), c("mesh", "DG", "fmla", "fold", "model.name")))
  ...
  save(fit, file = paste0("../Results/vessel/fit.", model.name, ".RData"))
  load(file = paste0("../Results/vessel/fit.", model.name, ".RData"))
...
df <- DG %>% filter(flag_id == new.data$flag_id[1])
new.data[,"vessel"] <- names(sort(table(df$vessel),decreasing=TRUE)[1])
rm(df)
...
DG %>% mutate(decade = floor(YrQtr / 10) * 10) %>%
  ...
  summarise(samp.size = n()) %>% ungroup() %>% filter(decade == 1980) %>%
  ggplot() +
  facet_wrap(~decade) +
...
index.df %>%
  ...
  labs(y = 'Std. Indices (Confedence Interval)', x ='Year') +
  theme_bw() + gg.theme +
  facet_grid(rows = vars(Region), scales='free')
