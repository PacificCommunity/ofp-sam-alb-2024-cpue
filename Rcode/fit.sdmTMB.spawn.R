...
load(file = "../Data/subsample_1954.Rdata") ## core vessel data
DG <- sub; rm(sub)
DG %<>% mutate(Response_variable = alb_cpue, month = as.numeric(month))

## confine to 10-25S in WCPO with 5 degree buffer
## only include Oct-Jan (peak spawning months)
DG %<>% filter(latd <= -5, latd >= -30, lond <= 235,
                 month %in% c(10, 11, 12, 1))

## convert to equal distant projection
crs_eqd = sp::CRS("+proj=tpeqd +lat_1=-17.5 +lon_1=170 +lat_2=-17.5 +lon_2=205 +datum=WGS84 +ellps=WGS84 +units=km +no_defs")
...
## define the mesh
grid_coords = expand.grid(lon=(129.5:235.5),lat=(-4.5:-30.5))
...
## define formula
fmla <- as.formula("Response_variable ~ as.factor(year) + as.factor(flag_id) + s(hbf, bs='bs', m=c(3,2))")
...
max(fit$gradients)
#fit <- run_extra_optimization(fit, nlminb_loops = 0, newton_loops = 2)
#save(fit, file = paste0("../Results/fit.extra.", model.name, ".RData"))
#load(file = paste0("../Results/fit.extra.", model.name, ".RData"))
fit$fit.time
fit$formula
...
## filter data to match model fit
new.data %<>% filter(latd <= -10, latd >= -25, lond <= 230)
...
new.data %>% filter(!is.na(Region)) %>% ggplot() +
  geom_point(aes(x = lond, y = latd)) +
  xlim(c(130, 210)) +
  geom_sf(data=coast_sf,alpha=0.5) + gg.theme +
  labs(x = "Longitude", y = "Latitude")

dir.create("../Figures/spawn")
ggsave(paste0('../Figures/spawn/spawning.area.png'), width=10, height=7,
       units='in', dpi=300)
graphics.off()
...
index.final <- pred.df %>%
  ...
  #group_by(YrQtr, Region) %>% summarise(rel_abund = sum(rel_abund)) %>%
...
index.final %<>% mutate(Model = "spawn") %>%
...
index.final %>% filter(Model %in% c("5x5", "spawn")) %>%
  group_by(Region, Model) %>%
  mutate(std = Index / mean(Index)) %>%
  ggplot(aes(YrQtr, std, group = as.factor(Model))) +
...
index.comb <- index.final %>% filter(Model == "spawn") %>%
...
## nominal
index.comb %>% filter(Region == "Region 1-ABCD") %>%
  group_by(Region, Model) %>%
  mutate(std = Index / mean(Index)) %>%
...
DG %>% mutate(decade = floor(YrQtr / 10) * 10) %>%
  ...
  group_by(decade) %>%
  mutate(prop.samp = 100*(samp.size / sum(samp.size))) %>%
  ungroup() %>%
  ggplot() +
  facet_wrap(~decade) +
  geom_tile(aes(x = lond, y = latd, fill = prop.samp)) +
  ...
  scale_fill_gradientn("% of Samples", colors=ltb.cpue.col) +
  theme_void() + gg.theme +
  labs(x = "Longitude", y = "Latitude")
...
index.df %>%
  ...
  labs(y = 'Std. Index (Confedence Interval)', x ='Year') +
  theme_bw() + gg.theme
