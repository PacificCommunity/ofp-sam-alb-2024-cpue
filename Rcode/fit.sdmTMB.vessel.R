...
cpue_data <- sf::st_as_sf(DG, coords = c("lon_eqd", "lat_eqd"), crs = crs_eqd)

## spatial blocking
#sb <- cv_spatial(x = cpue_data,
#                 k = length(folds), # number of folds
#                 hexagon = FALSE, # use square blocks
#                 rows_cols = c(15, 15), extend = 0.5,
#                 selection = "random",
#                 iteration = 100, seed = 1134)

k <- 1

#save(sb, file = paste0("../Data/sb.vessel.RData"))
load(file = paste0("../Data/sb.vessel.RData"))

## extract the fold indices from buffering object 
fold.nums <- sb$folds_list

fmla <- as.formula("Response_variable ~ as.factor(year) + as.factor(flag_id) + s(hbf, bs='bs', m=c(3,2)) + s(month, bs = 'cc', k = 12) + as.factor(vessel_id)")
fmla

#for(k in seq_len(length(folds))){
  ## extracting the training and testing indices
  ## this way works with folds_list list (but not folds_ids)
  #trainSet <- unlist(fold.nums[[k]][1]) # training set indices; first element
  testSet <- unlist(fold.nums[[k]][2]) # testing set indices; second element
  ...
  ## may need to turn off library(BlockCV), it can interfere with the mesh funs
  mesh_sdmTMB = make_mesh(dat, c("lon_eqd", "lat_eqd"), mesh = mesh_inla)
  mesh = add_barrier_mesh(mesh_sdmTMB,coast_eqd_sf,
                          range_fraction = 0.01,plot=FALSE)
  plot(mesh)

  graphics.off()
  rm(list=setdiff(ls(), c("mesh", "dat", "fmla", "fold")))
  ...
  fit<- sdmTMB(
    ...
    control = sdmTMBcontrol(newton_loops = 1, parallel = 1L))
  ...
  save(fit, file = paste0("../Results/vessel/fit.vessel.fold", fold, ".RData"))
  #load(file = paste0("../Results/vessel/fit.vessel.fold", fold, ".RData"))
